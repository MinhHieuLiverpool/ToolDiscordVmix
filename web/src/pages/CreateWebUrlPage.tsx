...
export default function CreateWebUrlPage() {
    ...
    const [shareType, setShareType] = useState<'game' | 'machines'>('machines')
    ...
    const handleOpenEditDialog = (link: SharedWebConfig) => {
        ...
        setShareType(link.share_type || 'machines')
        setGeneratedUrl('')
        setIsDialogOpen(true)
    }
    ...
    const handleSaveConfig = async () => {
        ...
        if (shareType === 'machines' && selectedMachines.length === 0) {
        ...
        try {
            if (editingUuid) {
                // Update
                const res = await updateSharedWebConfig(editingUuid, selectedFeatures, selectedMachines, selectedGame, shareType)
                if (res.success) {
                ...
                // Create
                const res = await createSharedWebConfig(selectedFeatures, selectedMachines, selectedGame, shareType)
    ...
    return (
        <div className="p-6 text-slate-800 dark:text-slate-100 flex flex-col gap-6">
            ...
            <div className="bg-white dark:bg-slate-900/45 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-800/80">
                <h3 className="text-xs font-black text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-4">Danh sách liên kết đang hoạt động</h3>
                                            ...
                                            <tr key={link.uuid}>
                                                ...
                                                <td>
                                                    <span className={`px-2 py-0.5 text-[9px] font-bold rounded-md ${
                                                        link.share_type === 'game' && link.selected_game && link.selected_game !== '__all__'
                                                            ? 'bg-amber-50 dark:bg-amber-950/20 text-amber-600 dark:text-amber-400'
                                                            : 'bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400'
                                                    }`}>
                                                        {link.share_type === 'game'
                                                            ? (games.find(g => g.id === link.selected_game)?.label || link.selected_game || 'Tất cả Game')
                                                            : 'Tất cả Game'
                                                        }
                                                    </span>
                                                </td>
                ...
                <div className="flex flex-col gap-5 text-slate-800 dark:text-slate-100 max-h-[80vh] overflow-y-auto pr-1">
                    ...
                    {generatedUrl ? (
                        ...
                        <>
                            ...
                            {/* Section 2: Choose share mode */}
                            <div>
                                <h3 className="text-xs font-black text-slate-400 uppercase tracking-wider mb-2.5">2. Chọn hình thức hiển thị máy</h3>
                                <div className="grid grid-cols-2 gap-3">
                                    <label 
                                        className={`flex items-center justify-center gap-2 px-4 py-3 rounded-xl border text-xs font-bold cursor-pointer select-none transition-all ${
                                            shareType === 'game'
                                                ? 'bg-purple-50 dark:bg-purple-950/20 border-purple-500 text-purple-600 dark:text-purple-400'
                                                : 'bg-slate-50/50 dark:bg-slate-900/30 border-slate-200 dark:border-slate-800/60 text-slate-600 dark:text-slate-400 hover:bg-slate-100/50 dark:hover:bg-slate-850/30'
                                        }`}
                                    >
                                        <input 
                                            type="radio"
                                            name="shareType"
                                            value="game"
                                            checked={shareType === 'game'}
                                            onChange={() => setShareType('game')}
                                            className="sr-only"
                                        />
                                        <span>Hiển thị theo Kênh Game</span>
                                    </label>
                                    <label 
                                        className={`flex items-center justify-center gap-2 px-4 py-3 rounded-xl border text-xs font-bold cursor-pointer select-none transition-all ${
                                            shareType === 'machines'
                                    ...
                                    >
                                        <input 
                                            ...
                                            name="shareType"
                                            value="machines"
                                            checked={shareType === 'machines'}
                                            onChange={() => setShareType('machines')}
                                            className="sr-only"
                                        />
                                        <span>Hiển thị theo Từng Máy chọn</span>
                                    </label>
                                </div>
                            </div>
                            ...
                            {shareType === 'game' && (
                                <>
                                    <hr className="border-slate-100 dark:border-slate-800/70" />
                                    <div>
                                        <h3 className="text-xs font-black text-slate-400 uppercase tracking-wider mb-2.5">3. Chọn kênh game mặc định</h3>
                                        <div className="relative">
                                            ...
                                            >
                                                {games.map((game) => (
                                                    <option key={game.id} value={game.id} className="dark:bg-slate-900 text-slate-700 dark:text-slate-300">
                                                        {game.label}
                                                    </option>
                                                ))}
                                            </select>
